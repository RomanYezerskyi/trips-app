...
export class MyCurrentTripsComponent  implements  OnInit, OnDestroy {
  ...
  getUserStartedTrips(): void {
    if (this.Skip <= this.totalTrips) {
      this.isSpinner = true;
      this.tripService.getUserStartedTrips(this.Take, this.Skip).pipe(takeUntil(this.unsubscribe$)).subscribe(
        response => {
          if (response != null) {
